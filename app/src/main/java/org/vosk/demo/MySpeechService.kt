...
class MySpeechService @SuppressLint("MissingPermission") constructor(
    private val recognizer: Recognizer,
    sampleRate: Float,
    private val recordFile: File
) {
    private val sampleRate = sampleRate.toInt()
    private val bufferSize: Int = Math.round(this.sampleRate * BUFFER_SIZE_SECONDS)
    ...
    fun startListening(listener: RecognitionListener): Boolean {
        ...
        recognizerThread = RecognizerThread(listener, recordFile)
    ...
    fun startListening(listener: RecognitionListener, timeout: Int): Boolean {
        ...
        recognizerThread = RecognizerThread(listener, timeout, recordFile)
        recognizerThread!!.start()
        return true
    }
    ...
    private inner class RecognizerThread(var listener: RecognitionListener, timeout: Int, recordFile: File) : Thread() {
        ...
        constructor(listener: RecognitionListener, recordFile: File) : this(listener, NO_TIMEOUT, recordFile)
        ...
        override fun run() {
            ...
            while (!interrupted()
                ...
                addBufferData(data, buffer.clone()) // Saving buffer data to later write to disk
            ...
            writeFile(data, recordFile) // Recording stopped, write recording to disk
            recorder.stop()
